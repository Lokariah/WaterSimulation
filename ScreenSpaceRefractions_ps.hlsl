...
float4 main(PostProcessingInput input) : SV_TARGET
{
	...
	float3 reflectionNormal = normalize(refract(normalize(viewSpacePos), viewSpaceNormal, gRior));
	//float3 reflectionNormal = normalize(reflect(normalize(viewSpacePos), viewSpaceNormal));

	//Calculating start and end point in view and screen space
	float4 rayStartPoint = float4(viewSpacePos + (reflectionNormal * 0), 1);
	float4 rayEndPoint = float4(viewSpacePos + (reflectionNormal * maxDistance), 1);
